...
Once a device-based run destination has been selected, select the “CleanroomBridging” build scheme.
...
If all went well, the Carthage build directory should contain the file `CleanroomBridging.framework`. If that file isn’t present, something went wrong with the build.
