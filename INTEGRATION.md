...
*Integration* is the act of embedding the `CleanroomBridging.framework` binary into your project, thereby exposing the API it provides to your code.

CleanroomBridging is built as a *Swift framework*, and as such, it has the following base platform requirements:
...
**This is the `swift2.3` branch of CleanroomBridging.** It uses **Swift 2.3** and **requires Xcode 8.0 beta 6** (or higher) to compile.
...
- **[Carthage integration](#carthage-integration)** — Explains how to use the [Carthage](https://github.com/Carthage/Carthage) dependency manager for adding CleanroomBridging to your project.

- **[Manual integration](#manual-integration)** — Demonstrates the steps for adding CleanroomBridging by embedding `CleanroomBridging.xcodeproj` within your own Xcode project. (**Note:** Manual integration is a bit more involved than using Carthage).
...
When building a framework for a platform with a simulator, Carthage creates a *universal binary*, allowing `CleanroomBridging.framework` to work in the simulator as well as on actual devices.
...
The command `carthage update` causes Carthage to download and build CleanroomBridging.

By default, `carthage update` builds all platform targets in a project. Normally, this is _not_ what you want; you’ll usually use CleanroomBridging on just one platform at a time.
...
Once Carthage is done building CleanroomBridging, you can use the `open` command to locate the framework binaries in Finder:
...
If all went well, the Carthage build directories should contain the file `CleanroomBridging.framework`. If that file isn’t present, something went wrong with the build.
...
Go back to Finder, select the file `CleanroomBridging.framework`, and then drag it into the list area directly below *Embedded Binaries*.
...
But before you start coding, skip to the [Adding the Swift import](#adding-the-swift-import) section to see how you can import CleanroomBridging for use in your Swift code.
...
From within the `Libraries` directory, issue the following commands to download CleanroomBridging:

```bash
git submodule add https://github.com/emaloney/CleanroomBridging
git submodule update --init --recursive
```

Next, you’re ready to [embed the `CleanroomBridging.xcodeproj` project file in your Xcode project](#2-embed-cleanroombridging-in-your-project).
...
```bash
git clone https://github.com/emaloney/CleanroomBridging
```
...
Once a device-based run destination has been selected, select the appropriate build scheme for the target platform: “CleanroomBridging-iOS”, “CleanroomBridging-OSX”, “CleanroomBridging-tvOS” or “CleanroomBridging-watchOS”.
...
1. Locate `CleanroomBridging.framework` in the Xcode project browser
...
Once you’ve done this for each framework, **_you’re all done integrating CleanroomBridging!_**
...
Want to learn more about CleanroomBridging? Check out [the README](https://github.com/emaloney/CleanroomBridging/blob/master/README.md).
