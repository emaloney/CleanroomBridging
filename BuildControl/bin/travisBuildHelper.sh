...
while [[ $THIS_TRY < $MAXIMUM_TRIES ]]; do
	...
	( set -o pipefail && xcodebuild -project CleanroomBridging.xcodeproj -configuration Release ONLY_ACTIVE_ARCH=YES -scheme CleanroomBridging-$PLATFORM -destination "$DESTINATION" -destination-timeout 300 $XCODE_ACTION 2>&1 | tee "CleanroomBridging-$PLATFORM-$OPERATION.log" | xcpretty )
	XCODE_RESULT="${PIPESTATUS[0]}"
	if [[ "$XCODE_RESULT" == "0" ]]; then
		rm "CleanroomBridging-$PLATFORM-$OPERATION.log"
		exit 0
