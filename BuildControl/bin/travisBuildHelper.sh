...
while [[ $THIS_TRY < $MAXIMUM_TRIES ]]; do
	...
	( set -o pipefail && xcodebuild -project CleanroomBridging.xcodeproj -configuration Debug -scheme "CleanroomBridging" -destination "$DESTINATION" -destination-timeout 300 $XCODE_ACTION 2>&1 | tee "CleanroomBridging-$PLATFORM-$OPERATION.log" | xcpretty )
